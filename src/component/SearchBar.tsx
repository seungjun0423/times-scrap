...
function SearchBar() {
	...
	useLayoutEffect(() => {
		if( page === 'home' ){
			...
			const headlineState = filterScrap.headline ? filterScrap.headline:"전체 헤드라인";
			const dateState = filterScrap.date ? filterScrap.date:"전체 날짜";
			
			const nationState = 
			// 국가를 선택하지 않은 경우
			filterScrap.nation.length === 0 ? "전체 국가" :
			// 기본 상태인 경우 
			typeof(filterScrap.nation) === "string" ? filterScrap.nation : 
			// 1개 이상을 선택한 경우
			filterScrap.nation.length > 1 ? filterScrap.nation[0].nation+` 외 ${filterScrap.nation.length-1}개`: 
			// 1개만 선택한 경우
			filterScrap.nation[0].nation;
