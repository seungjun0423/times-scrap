...
// const getFilterd = ( date, headLine, nation ): Promise<> => {
// 	const req = axios.get(
...
const getTodayHeadline = ( date: string ): Promise<TarticleData[]> => {
	const req = axios.get(
		`${VITE_API_URL}fq=pub_date:(${date}) AND Section Name Values:('Today's Headlines')&api-key=${VITE_API_KEY}`
	).then( res => res.data.response.docs);
	return req;
};
